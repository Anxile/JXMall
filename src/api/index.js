...
//axios.get()和axios.post()是axios库提供的两个方法，用于发送GET和POST请求。
//这两个方法的作用是相同的，都是发送HTTP请求到指定的URL，并返回Promise对象以便处理响应数据。
//axios({})是通用的方式，可以满足发送任意类型的HTTP请求的需求，包括GET、POST、PUT、DELETE等。

//三级联动接口
export const reqCategoryList = () => requests.get("/product/getBaseCategoryList");  //无参数
